...
export default function ChatsPage() {
  ...
  const loadMatches = useCallback(async (currentUser) => {
    ...
    const ids = (matchRows || []).map((row) => row.user_one === currentUser.id ? row.user_two : row.user_one);
  ...
  useEffect(() => {
    ...
    const supabase = getSupabaseBrowser();
    supabase.from('messages').select('*').eq('match_id', selectedMatchId).order('created_at', { ascending: true }).then(({ data }) => setMessages(data || []));
  }, [selectedMatchId]);
  ...
  async function sendMessage(e) {
    ...
    const supabase = getSupabaseBrowser();
    const payload = { match_id: selectedMatchId, sender_id: user.id, body: text.trim() };
  ...
  return (
    <RequireAuth onUserLoaded={(loadedUser) => { setUser(loadedUser); loadMatches(loadedUser); }}>
        ...
        <div className="dc-chat-layout">
          <section className="dc-card dc-panel">
            ...
            <div className="dc-stack">
              {!matches.length ? <div className="dc-notice">When it’s a match, your chats show up here.</div> : null}
              {matches.map((match) => (
                <button key={match.id} className={`dc-chat-row ${selectedMatchId === match.id ? 'active' : ''}`} onClick={() => setSelectedMatchId(match.id)}>
                  <img src={match.otherProfile.selfie_url || '/assets/user-lola.png'} alt={match.otherProfile.first_name} />
                  <div>
            ...
            {!selectedMatch ? (
              ...
              <>
                ...
                <div className="dc-chat-thread">
                  {!messages.length ? (
                    <>
                      <div className="dc-msg them">Okay be honest… do we actually look alike?</div>
                      <div className="dc-msg me">This match is dangerously familiar.</div>
                    </>
                  ) : messages.map((msg, index) => (
                    <div key={`${msg.created_at || index}-${index}`} className={`dc-msg ${msg.sender_id === user?.id ? 'me' : 'them'}`}>
                      {msg.body}
                    </div>
                  ))}
                </div>
