...
export default function DiscoverPage() {
  ...
  async function handleLike(target) {
    ...
    const pairKey = makePairKey(user.id, target.id);
    ...
    if (matchError) {
      setMessage(`Saved your like for ${target.first_name}, but couldn’t open the chat just yet.`);
      setCandidates((prev) => prev.filter((item) => item.id !== target.id));
      return;
    }

    setMessage(`It’s a match with ${target.first_name}. Opening chat...`);
    setCandidates((prev) => prev.filter((item) => item.id !== target.id));

    router.push(`/chats?match=${matchRow.id}`);
  }
  ...
  async function handlePass(target) {
    ...
    setCandidates((prev) => prev.filter((item) => item.id !== target.id));
    setMessage(`Noted. ${target.first_name} is off your radar.`);
  }
        ...
        <section className="dc-card dc-panel">
          <div className="dc-panel-top">
            <div>
              ...
              <h2>Find your next DoppelCrush</h2>
              <p className="dc-muted">
                Scroll the familiar. Pick your favourite. See where the vibe goes.
              </p>
            </div>

            <div className="dc-pill-toggle">
              <button
                className={mode === 'doppel' ? 'active' : ''}
          ...
          {mode === 'doppel' ? (
            <div className="dc-notice">
              Doppel Mode is on. Expect familiar faces, matching energy, and suspiciously good taste.
            </div>
          ) : (
            <div className="dc-notice">
              Chaos Mode is on. Plot twists only.
            </div>
          )}

          {message ? <div className="dc-notice">{message}</div> : null}
          {loading ? <div className="dc-notice">Loading cute people...</div> : null}

          {!loading && scoredCandidates.length === 0 ? (
            <div className="dc-notice">
              You’ve run out of fresh faces for now. Bring more people in or come back for another scroll.
            </div>
          ) : null}
