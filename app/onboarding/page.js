...
export default function OnboardingPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (photoFile) {
      ...
      const upload = await supabase.storage.from('selfies').upload(filePath, photoFile, { upsert: true });
      if (upload.error) {
        setLoading(false);
          ...
          <div className="dc-card dc-auth-card">
            ...
            <form className="dc-form" onSubmit={handleSubmit}>
              <div className="dc-upload-box">
                {preview ? <img className="dc-selfie-preview" src={preview} alt="Selfie preview" /> : <div className="dc-upload-placeholder">Pick your best selfie</div>}
              ...
              <div className="dc-two-col">
                ...
                <div>
                  <label>Age</label>
                  <input type="number" min="18" max="99" value={form.age} onChange={(e) => update('age', e.target.value)} required />
                </div>
              </div>
              ...
              <div className="dc-two-col">
                ...
                <div>
                  ...
                  <select value={form.looking_for} onChange={(e) => update('looking_for', e.target.value)}>
              ...
              <input value={form.location} onChange={(e) => update('location', e.target.value)} placeholder="London" />

              <label>Bio</label>
              <textarea value={form.bio} onChange={(e) => update('bio', e.target.value)} placeholder="Funny, flirty, chaotic, iconic — your call." />

              <label>Mode</label>
              <div className="dc-button-row">
                <button type="button" className={`dc-btn ${form.mode_preference === 'doppel' ? 'dc-btn-primary' : 'dc-btn-light'}`} onClick={() => update('mode_preference', 'doppel')}>Doppel Mode</button>
                <button type="button" className={`dc-btn ${form.mode_preference === 'chaos' ? 'dc-btn-primary' : 'dc-btn-light'}`} onClick={() => update('mode_preference', 'chaos')}>Chaos Mode</button>
              </div>

              {error ? <div className="dc-error">{error}</div> : null}
              <button className="dc-btn dc-btn-primary" disabled={loading}>{loading ? 'Saving profile…' : 'Show me my matches'}</button>
            </form>
          </div>
        </div>
      </main>
